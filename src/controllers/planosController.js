...
class PlanosController {
    async create(req, res) {
        try {
            ...
            if (!nome || !periodo || !preco || !duracaoDias || !beneficios || beneficios.length === 0) {
                return res.status(400).json({ error: "Nome, período, preço, duração e benefícios são obrigatórios" });
            }
    ...
    async get(req, res) {
        try {
            const { id } = req.params;
            const [rows] = await pool.execute("SELECT * FROM planos WHERE id = ?", [id]);
